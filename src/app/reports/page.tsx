...
export default function ReportsPage() {
  ...
  const exportReport = (reportType: string) => {
    ...
    if (exportFormat === 'csv') {
      ...
      downloadFile(csvContent, `relatorio_${reportType}_${new Date().toISOString().split('T')[0]}.csv`, 'text/csv');
    } else if (exportFormat === 'excel') {
      // Simular exportação Excel
      alert(`Relatório ${reportType} exportado para Excel!`);
    } else {
      // Simular exportação PDF
      alert(`Relatório ${reportType} exportado para PDF!`);
    }
  };
  ...
  const generateCSV = (data: any) => {
    ...
    const rows = [
      ['Receita Total', `R$ ${data.metrics.totalRevenue.toLocaleString('pt-BR')}`, selectedPeriod + ' dias'],
      ['Despesas Totais', `R$ ${data.metrics.totalExpenses.toLocaleString('pt-BR')}`, selectedPeriod + ' dias'],
      ['Lucro Líquido', `R$ ${data.metrics.netProfit.toLocaleString('pt-BR')}`, selectedPeriod + ' dias'],
      ['Margem de Lucro', `${data.metrics.profitMargin.toFixed(2)}%`, selectedPeriod + ' dias'],
      ['Total de Clientes', data.metrics.totalClients.toString(), 'Total'],
      ['Clientes Ativos', data.metrics.activeClients.toString(), 'Total'],
      ['Taxa de Conversão', `${data.metrics.conversionRate.toFixed(2)}%`, 'Total'],
      ['Total de Oportunidades', data.metrics.totalTasks.toString(), 'Total'],
      ['Oportunidades Fechadas', data.metrics.closedTasks.toString(), 'Total']
    ];
  ...
  const renderReport = () => {
    switch (activeReport) {
      ...
      case 'die':
        return <DIEReport />;
      case 'sales':
        return <div className="text-white text-center py-20">Relatório de Vendas em desenvolvimento...</div>;
      case 'financial':
        return <div className="text-white text-center py-20">Relatório Financeiro em desenvolvimento...</div>;
      case 'performance':
        return <div className="text-white text-center py-20">Relatório de Performance em desenvolvimento...</div>;
      case 'growth':
        return <div className="text-white text-center py-20">Análise de Crescimento em desenvolvimento...</div>;
      default:
        return null;
    }
  };
