...
export default function AdminPage() {
  const { user, isLoading, pendingUsers, approveUser, rejectUser, isAdmin } = useStackAuth();
  ...
  const handleApprove = async (userId: string) => {
    ...
    try {
      ...
      if (success) {
        ...
        console.log('Usuário aprovado:', userId);
      }
    } catch (error) {
                ...
                {pendingUsers.map((pendingUser) => (
                  <div key={pendingUser.id} className="flex items-center justify-between p-4 bg-gray-700 rounded-lg">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h3 className="text-white font-medium">{pendingUser.displayName}</h3>
